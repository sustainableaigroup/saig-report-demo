...
(() => {
  ...
  const currentPdf = () => {
    ...
    if (pathname.endsWith("/")) return "index.pdf";
    const filename = pathname.split("/").pop() || "index.html";
    return filename.replace(/\.html?$/i, ".pdf");
  };

  const addResources = () => {
    const toc = document.querySelector("#TOC") || document.querySelector('nav[role="doc-toc"]');
  ...
  const addEyebrows = () => {
    let auto = 0;
    document.querySelectorAll("main.content section.level1 > h1").forEach((heading) => {
      if (heading.closest(".saig-cover") || heading.id === "references" || heading.id === "refs") return;
      if (heading.previousElementSibling?.classList?.contains("chapter-eyebrow")) return;
  ...
  const brandCaptions = () => {
    document.querySelectorAll("figcaption, table caption").forEach((caption) => {
      if (caption.querySelector(".saig-caption-label")) return;
      const nodes = Array.from(caption.childNodes);
      const text = caption.textContent || "";
      const match = text.match(/^\s*((?:Figure|Table)\s+\d+[:.]?)/i);
      if (!match) return;
  ...
  const wrapTables = () => {
    document.querySelectorAll("main.content table").forEach((table) => {
      if (table.parentElement?.classList?.contains("saig-table-scroll")) return;
